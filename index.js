...
app.get("/getkey", async (req, res, next) => {
	...
	if (!data) {
		...
		try {
			await data.save()
			return res.render("getkey", {ip: GenerateToken(data.IP),checkpoint: 1, token: process.env.key})
		} catch(err) {
			return res.redirect("/")
		}
		return
	...
	if (!data.Checkpoint2) {
		return res.render("getkey", {ip: GenerateToken(data.IP), checkpoint: 2, token: process.env.key})
	}
	next()
})
